...
- _annotation_files/_
Contains the annotation files in `.txt` format for the analysis. In this folder, you can place your own annotation file(s), see the provided Example_annotation_list_hg38.txt for the required format.
...
bin/
│
├── app.py (or your Flask main file)
├── templates/
│   ├── base.html
│   └── home.html
└── static/
...
```
python3 path/to/SarcDBase/bin/sv.py -file path/to/${sample}.vcf -annotation path/to/SarcDBase/annotation_files/user_annotation_list.txt -outfile path/to/SarcDBase/results/${sample}*_sv.txt -filter PASS
```
The script below utilizes a for loop to provide correct sample names, modify as needed.

```
for file in /path/to/*sv.vcf; do sample=$(basename "$file" | sed 's/_sv.vcf$//'); echo "running vcf file $sample"; python /path/to/SarcDBase-1.0.0/bin/sv.py -file $file -annotation /path/to/SarcDBase-1.0.0/annotation_files/user_annotation_list.txt -outfile /path/to/SarcDBase-1.0.0/results/$sample/${sample}_sv.txt -filter PASS; done
```
...
```
python3 path/to/SarcDBase/bin/point_mutations.py -file path/to/${sample}.vcf -annotation path/to/SarcDBase/annotation_files/user_annotation_list.txt -outfile path/to/SarcDBase/results/${sample}*_snv.txt -filter PASS
```
The script below utilizes a for loop to provide correct sample names, modify as needed.

```
for file in /path/to/*_snv.vcf; do sample=$(basename "$file" | sed 's/_snv.vcf$//'); echo "running vcf file $sample"; python /path/to/SarcDBase-1.0.0/bin/point_mutations.py -file $file -annotation /path/to/SarcDBase-1.0.0/annotation_files/user_annotation_list.txt -outfile /path/to/SarcDBase-1.0.0/results/$sample/${sample}_snv.txt -filter PASS; done
```
...
-annotation /path/to/annotation_files/user_annotation_list.txt \
